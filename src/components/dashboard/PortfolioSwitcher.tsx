...
import { Briefcase, ChevronDown, Plus, Check, Lock } from 'lucide-react'
...
export function PortfolioSwitcher({ activeId, ownPortfolios, sharedPortfolios }: Props) {
  ...
  const active =
    ownPortfolios.find(p => p.id === activeId) ??
    sharedPortfolios.find(p => p.id === activeId)
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        >
          <Briefcase className="w-3 h-3" style={{ color: active.color }} />
        </div>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 rounded-xl border border-zinc-700/60 bg-zinc-900 shadow-2xl shadow-black/50 overflow-hidden z-50">
            ...
            <>
              ...
              {ownPortfolios.map(p => (
                <button
                  ...
                  disabled={!!switching}
                  className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-zinc-800 transition-colors disabled:opacity-50"
                >
                  <div
                  ...
                  <span className={cn('flex-1 text-sm text-left truncate', p.id === activeId ? 'text-zinc-100 font-medium' : 'text-zinc-400')}>
                    {p.name}
                  </span>
                  {p.id === activeId && <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
              ...
              {sharedPortfolios.map(p => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <p className={cn('text-sm truncate', p.id === activeId ? 'text-zinc-100 font-medium' : 'text-zinc-400')}>{p.name}</p>
                    <p className="text-[10px] text-zinc-600 truncate">by {p.owner_name}</p>
                  </div>
                  {p.id === activeId && <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
                  {switching === p.id && <div className="w-3.5 h-3.5 border border-zinc-500 border-t-zinc-300 rounded-full animate-spin shrink-0" />}
                </button>
          ...
          <div className="p-2 border-t border-zinc-800 mt-1">
            <Link
              href="/portfolios"
